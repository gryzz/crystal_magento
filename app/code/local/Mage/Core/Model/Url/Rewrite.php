...
class Mage_Core_Model_Url_Rewrite extends Mage_Core_Model_Abstract
{
    ...
    public function rewrite(Zend_Controller_Request_Http $request=null, Zend_Controller_Response_Http $response=null)
    {
        ...
        switch (Mage::app()->getStore($this->getStoreId())->getCode()) {
            case 'ru': $from_store = 'ua'; break;
            case 'ua': $from_store = 'ru'; break;
        }

        /**
         * Try to find rewrite by request path at first, if no luck - try to find by id_path
         */
        if (!$this->getId() && isset($from_store)) {
            try {
                $fromStoreId = Mage::app()->getStore($from_store)->getId();
            }
            catch (Exception $e) {
                return false;
            }
